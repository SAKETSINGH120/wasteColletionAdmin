...
import { getAllDeliveryBoys } from "../../Services/DeliveryBoyApi";
import { AssignPickupBoy } from "../../Services/LatestPickupApi";
...
export default function RequestPickupView() {
  ...
  const openAssignModal = async () => {
    ...
    // fetch first page of delivery boys
    try {
      setLoadingBoys(true);
      ...
      console.log(res, "dddddddddfff");
      if (res?.status) {
        const list = (res.data || []).map((b) => ({
          label: b.name || b.mobile || b._id,
          value: b._id,
        }));
        setDeliveryBoys(list);
      } else {
        toast.error(res?.message || "Failed to fetch delivery boys");
      }
    } catch (err) {
      console.error(err);
  ...
  const handleAssignConfirm = async () => {
    if (!selectedBoy) return toast.error("Please select a pickup boy");
    try {
      const payload = { requestPickupId: id, pickupBoyId: selectedBoy };
      const res = await AssignPickupBoy(payload);
      if (res?.status) {
        ...
        toast.error(res?.message || "Failed to assign pickup boy");
      }
    } catch (err) {
      console.error(err);
      toast.error("Error assigning pickup boy");
    }
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <motion.div whileTap={{ scale: 0.95 }} whileHover={{ scale: 1.03 }}>
              <button
                ...
                className="bg-white border border-green-600 text-[#1F4926] px-4 py-2 rounded-xl font-semibold hover:bg-green-50 transition-all duration-200"
              >
                Assign Pickup Boy
              </button>
            </motion.div>
              ...
              <div className="w-full mt-3 grid grid-cols-2 gap-2">
                ...
                >
                  <div className="space-y-3">
                    ...
                    <p className="text-sm text-gray-600">Select Pickup Boy</p>
                    <Select
                      showSearch
                      placeholder={
                        loadingBoys ? "Loading..." : "Select a pickup boy"
                      }
                      options={deliveryBoys}
                      onChange={(val) => setSelectedBoy(val)}
