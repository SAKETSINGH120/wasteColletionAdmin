...
import { getSubCategoryLogsApi } from "../../Services/SubCategoryApi";
import { EyeIcon } from "@heroicons/react/24/outline";
import MoreVertIcon from "@mui/icons-material/MoreVert";
import Chip from "@mui/material/Chip";
import xlsx from "json-as-xlsx";

export default function SubCategoryLog() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getSubCategoryLogsApi({
        page,
        rowsPerPage,
        searchQuery,
      });
      if (result?.status) {
        toast.success("Subcategory logs fetched successfully!");
        const transformedData = (result.data || []).map((item) => ({
          ...item,
          id: item._id,
          subcategoryId: item.subcategory?._id,
          subcategoryName: item.subcategory?.name,
          imagePath: item.subcategory?.image,
          categoryId: item.subcategory?.category,
        ...
        toast.error(result?.message || "Failed to fetch subcategory logs.");
      }
    } catch (error) {
      console.error("Error fetching subcategory logs:", error);
      toast.error("Error fetching subcategory logs.");
    } finally {
      setLoading(false);
    }
  ...
  const exportFunc = async (allLogsData) => {
    if (allLogsData.length < 1) {
      return toast.error("Subcategory logs list is empty!");
    }
    setIsExporting(true);
    const settings = {
      fileName: "SubCategory_Logs",
      extraLength: 3,
      writeMode: "writeFile",
      writeOptions: {},
      RTL: false,
    ...
    const data = [
      {
        sheet: "SubCategory Logs",
        columns: [
          { label: "Log ID", value: (row) => row?._id || "" },
          {
            label: "Subcategory ID",
            value: (row) => row?.subcategoryId || "N/A",
          },
          {
            label: "Subcategory Name",
            value: (row) => row?.subcategoryName || "N/A",
          },
    ...
    try {
      xlsx(data, settings);
      toast.success("Subcategory logs exported to Excel successfully!");
    } catch (error) {
      console.error("Error exporting subcategory logs to Excel:", error);
      toast.error("Failed to export subcategory logs to Excel.");
    } finally {
    ...
    <div className="p-6 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        >
          <div className="flex items-center gap-4">
            <input
              type="text"
              placeholder="Search subcategory logs..."
              value={search}
              ...
              aria-label="Search subcategory logs"
            />
        ...
        <motion.button
          ...
          aria-label="Export subcategory logs to Excel"
        >
          {isExporting ? (
      ...
      >
        <table
          ...
          aria-label="subcategory logs table"
        >
          <thead>
            ...
            {data.length === 0 ? (
              ...
              data.map((row, index) => (
                ...
                >
                  ...
                  {/* <td className="py-3 px-4 text-gray-600">
                    {row.subcategoryId || "N/A"}
                  </td> */}
                  <td className="py-3 px-4 text-gray-600">
                    {row.subcategoryName || "N/A"}
                  </td>
                  {/* <td className="py-3 px-4 text-gray-600">
                    {row.categoryId || "N/A"}
                  </td> */}
                  ...
                  <td className="py-3 px-4 text-center">
                    ...
                    >
                      <button
                        onClick={() => {
                          navigate(`SubCategoryLogView/${row.id}`); // Adjust route as needed for detailed view
                          handleMenuClose();
                        }}
      ...
      {totalRecord > rowsPerPage && (
        <div className="mt-6 flex justify-center">
          <nav aria-label="Pagination for subcategory logs">
