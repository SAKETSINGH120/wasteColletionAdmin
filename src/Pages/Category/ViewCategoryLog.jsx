...
import { getCategoryLogByIdApi } from "../../Services/CategoryApi";
...
export default function ViewCategoryLog() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const result = await getCategoryLogByIdApi(id);
      if (result?.status) {
        toast.success(result.message || "Category log fetched successfully!");
        setData(result.data);
      } else {
        toast.error(result?.message || "Failed to fetch category log.");
      }
    } catch (error) {
      console.error("Error fetching category log:", error);
      toast.error("Error fetching category log.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getImageUrl = (path) => {
    if (!path) return "/defaultCategory.jpg"; // fallback in public
    ...
    <div className="p-8 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
      ...
      >
        <div className="flex justify-between items-center mb-8 pb-6 border-b-2 border-gray-200 px-6 pt-6">
          <div>
            <h1 className="text-3xl font-bold text-[#1F4926] tracking-wide">
              Category Log #{String(data._id).slice(-6)}
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 pb-6">
          {/* Left: Category Image and Details */}
          <div className="col-span-1 bg-white p-4 rounded-xl shadow-md">
            <div className="flex flex-col items-center text-center gap-3">
              <img
                src={`${BASE_URL}/${data.category?.image}`}
                alt={data.category?.name || "Category Image"}
                onError={(e) => (e.target.src = "/defaultCategory.jpg")}
              ...
              <div className="w-full mt-3 space-y-3">
                ...
                <InfoRow
                  IconComp={ImageIcon}
                  title="Category ID"
                  value={data.category?._id?.slice(-6) || "-"}
                />
              </div>
            </div>
          </div>
