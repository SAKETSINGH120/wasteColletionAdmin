...
import { getAllCategory, CategoryDelete } from "../../Services/CategoryApi";
...
export default function CategoryList() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getAllCategory({ page, rowsPerPage, searchQuery });
      if (result?.status) {
  ...
  const exportFunc = async (allLeadsData) => {
    if (allLeadsData.length < 1) {
      return toast.error("Category list is empty!");
    }
    setIsExporting(true);
    const settings = {
      fileName: "WasteCollection_Categories",
      extraLength: 3,
      writeMode: "writeFile",
      writeOptions: {},
      RTL: false,
                ...
                <StyledTableRow key={row.id}>
                  ...
                  <StyledTableCell align="center">
                    <IconButton
                      ...
                      aria-label={`Actions for category ${row.name || row.id}`}
                    >
                      <MoreVertIcon />
                    </IconButton>
                    <Menu
      ...
      {totalRecord > rowsPerPage && (
        <Stack spacing={2} alignItems="center" marginTop={6}>
          <Pagination
            ...
            boundaryCount={1}
            siblingCount={1}
            aria-label="Pagination for category list"
          />
        </Stack>
      )}
    </div>
