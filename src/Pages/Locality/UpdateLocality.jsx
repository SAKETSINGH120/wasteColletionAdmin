...
import { getLocalityById, updateLocality } from "../../Services/LocalityApi";
...
const UpdateLocality = () => {
      ...
      try {
        ...
        if (res.status) {
          setFormData({
            ...
            availablePincodes: res.data.availablePincodes || [],
            locality: res.data.locality || "",
            status: res.data.status ?? true,
          });
        } else {
          toast.error(res.message || "Failed to fetch locality");
          navigate(-1);
        }
      } catch (error) {
        toast.error(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateLocality(id, dataToSend);
      if (res.status) {
    ...
    <div className="">
      <div className="bg-white p-6 max-w-9xl rounded-xl shadow-xl">
        <form onSubmit={handleSubmit}>
          ...
          >
            {loading ? "Updating..." : "Update Locality"}
          </button>
        </form>
      </div>
    </div>
