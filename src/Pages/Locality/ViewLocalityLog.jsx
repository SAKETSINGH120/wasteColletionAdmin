...
import { getLocalityLogByIdApi } from "../../Services/LocalityApi";
...
export default function ViewLocalityLog() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const result = await getLocalityLogByIdApi(id);
      if (result?.status) {
        toast.success(result.message || "Locality log fetched successfully!");
        setData(result.data);
      } else {
        toast.error(result?.message || "Failed to fetch locality log.");
      }
    } catch (error) {
      console.error("Error fetching locality log:", error);
      toast.error("Error fetching locality log.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex justify-between items-center mb-8 pb-6 border-b-2 border-gray-200 px-6 pt-6">
          <div>
            <h1 className="text-3xl font-bold text-[#1F4926] tracking-wide">
              Locality Log #{String(data._id).slice(-6)}
            </h1>
          ...
          <div className="col-span-1 bg-white p-4 rounded-xl shadow-md">
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <div className="w-full mt-3 space-y-3">
                <InfoRow
                  IconComp={LocationOnIcon}
                  title="Locality ID"
                  value={data.locality?._id?.slice(-6) || "-"}
                />
                <InfoRow
                  IconComp={PinDropIcon}
                  title="Pincode"
                  value={data.locality?.pincode || "-"}
                />
                <InfoRow
                  IconComp={LocationOnIcon}
                  title="Locality Name"
                  value={data.locality?.locality || "-"}
                />
              </div>
            </div>
          </div>
