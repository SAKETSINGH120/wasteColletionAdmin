...
import { getLocalityLogsApi } from "../../Services/LocalityApi";
import { EyeIcon } from "@heroicons/react/24/outline";
import MoreVertIcon from "@mui/icons-material/MoreVert";
import Chip from "@mui/material/Chip";
import xlsx from "json-as-xlsx";

export default function LocalityLog() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getLocalityLogsApi({
        page,
        rowsPerPage,
        searchQuery,
      });
      if (result?.status) {
        toast.success("Locality logs fetched successfully!");
        const transformedData = (result.data || []).map((item) => ({
          ...item,
          id: item._id,
          localityId: item.locality?._id,
          pincode: item.locality?.pincode,
          localityName: item.locality?.locality,
        ...
        toast.error(result?.message || "Failed to fetch locality logs.");
      }
    } catch (error) {
      console.error("Error fetching locality logs:", error);
      toast.error("Error fetching locality logs.");
    } finally {
      setLoading(false);
    }
  ...
  const exportFunc = async (allLogsData) => {
    if (allLogsData.length < 1) {
      return toast.error("Locality logs list is empty!");
    }
    setIsExporting(true);
    const settings = {
      fileName: "Locality_Logs",
      extraLength: 3,
      writeMode: "writeFile",
      writeOptions: {},
      RTL: false,
    ...
    const data = [
      {
        ...
        columns: [
          ...
          { label: "Locality ID", value: (row) => row?.localityId || "N/A" },
          { label: "Pincode", value: (row) => row?.pincode || "N/A" },
          {
            label: "Locality Name",
            value: (row) => row?.localityName || "N/A",
          },
          {
            label: "Updated By",
    ...
    try {
      xlsx(data, settings);
      toast.success("Locality logs exported to Excel successfully!");
    } catch (error) {
      console.error("Error exporting locality logs to Excel:", error);
      toast.error("Failed to export locality logs to Excel.");
    } finally {
    ...
    <div className="p-6 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        >
          <div className="flex items-center gap-4">
            <input
              type="text"
              placeholder="Search locality logs..."
              value={search}
              ...
              aria-label="Search locality logs"
            />
        ...
        <motion.button
          ...
          aria-label="Export locality logs to Excel"
        >
          {isExporting ? (
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              data.map((row, index) => (
                ...
                >
                  <td className="py-6 px-5 text-gray-600">
                  ...
                  <td className="py-1 px-4 text-center">
                    ...
                    >
                      <button
                        onClick={() => {
                          navigate(`LocalityLogView/${row.id}`); // Adjust route as needed for detailed view
                          handleMenuClose();
                        }}
                        className="flex items-center gap-2 px-4 py-1 text-gray-700 hover:bg-gray-100 w-full text-left"
                      >
      ...
      {totalRecord > rowsPerPage && (
        <div className="mt-6 flex justify-center">
          <nav aria-label="Pagination for locality logs">
