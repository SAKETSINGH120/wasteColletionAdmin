...
import { getDailyTipsLogsApi } from "../../Services/DailyTipsApi";
import { EyeIcon } from "@heroicons/react/24/outline";
import MoreVertIcon from "@mui/icons-material/MoreVert";
import Chip from "@mui/material/Chip";
import xlsx from "json-as-xlsx";

export default function DailyTipsLog() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getDailyTipsLogsApi({
        page,
        rowsPerPage,
        searchQuery,
      });
      if (result?.status) {
        toast.success("Daily tips logs fetched successfully!");
        const transformedData = (result.data || []).map((item) => ({
          ...item,
          id: item._id,
          tipId: item.dailyTips?._id,
          tipTitle: item.dailyTips?.title,
          showDateFormatted: item.dailyTips?.showDate
            ? new Date(item.dailyTips.showDate).toLocaleDateString()
            : "N/A",
        ...
        toast.error(result?.message || "Failed to fetch daily tips logs.");
      }
    } catch (error) {
      console.error("Error fetching daily tips logs:", error);
      toast.error("Error fetching daily tips logs.");
    } finally {
      setLoading(false);
    }
  ...
  const exportFunc = async (allLogsData) => {
    if (allLogsData.length < 1) {
      return toast.error("Daily tips logs list is empty!");
    }
    setIsExporting(true);
    const settings = {
      fileName: "DailyTips_Logs",
      extraLength: 3,
      writeMode: "writeFile",
      writeOptions: {},
      RTL: false,
    ...
    const data = [
      {
        sheet: "Daily Tips Logs",
        columns: [
          { label: "Log ID", value: (row) => row?._id || "" },
          { label: "Tip ID", value: (row) => row?.tipId || "N/A" },
          { label: "Tip Title", value: (row) => row?.tipTitle || "N/A" },
          {
            label: "Show Date",
            value: (row) => row?.showDateFormatted || "N/A",
          },
          {
            label: "Updated By",
    ...
    try {
      xlsx(data, settings);
      toast.success("Daily tips logs exported to Excel successfully!");
    } catch (error) {
      console.error("Error exporting daily tips logs to Excel:", error);
      toast.error("Failed to export daily tips logs to Excel.");
    } finally {
    ...
    <div className="p-6 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-4">
          <input
            type="text"
            placeholder="Search daily tips logs..."
            value={search}
            ...
            aria-label="Search daily tips logs"
          />
          <button
            onClick={() => {
        ...
        <motion.button
          ...
          aria-label="Export daily tips logs to Excel"
        >
          {isExporting ? (
      ...
      >
        <table
          ...
          aria-label="daily tips logs table"
        >
          <thead>
            <tr className="bg-gradient-to-t from-[#45d85e] to-[#1F4926] text-white text-sm font-semibold uppercase tracking-wider">
              <th className="py-3 px-4 text-left">S.No</th>
              {/* <th className="py-3 px-4 text-left">Tip ID</th> */}
              <th className="py-3 px-4 text-left">Tip Title</th>
              <th className="py-3 px-4 text-left">Show Date</th>
                ...
                >
                  ...
                  <td className="py-3 px-4 text-center">
                    ...
                    >
                      <button
                        onClick={() => {
                          navigate(`DailyLogView/${row.id}`); // Adjust route as needed for detailed view
                          handleMenuClose();
                        }}
      ...
      {totalRecord > rowsPerPage && (
        <div className="mt-6 flex justify-center">
          <nav aria-label="Pagination for daily tips logs">
