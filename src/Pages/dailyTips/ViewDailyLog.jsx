...
import { getDailyTipLogByIdApi } from "../../Services/DailyTipsApi";
...
export default function ViewDailyTipLog() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const result = await getDailyTipLogByIdApi(id);
      if (result?.status) {
        toast.success(result.message || "Daily tip log fetched successfully!");
        setData(result.data);
      } else {
        toast.error(result?.message || "Failed to fetch daily tip log.");
      }
    } catch (error) {
      console.error("Error fetching daily tip log:", error);
      toast.error("Error fetching daily tip log.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getImageUrl = (path) => {
    if (!path) return "/defaultImage.jpg"; // fallback in public
    ...
    <div className="p-8 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
      ...
      >
        <div className="flex justify-between items-center mb-8 pb-6 border-b-2 border-gray-200 px-6 pt-6">
          <div>
            <h1 className="text-3xl font-bold text-[#1F4926] tracking-wide">
              Daily Tip Log #{String(data._id).slice(-6)}
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 pb-6">
          {/* Left: Daily Tip Media and Details */}
          <div className="col-span-1 bg-white p-4 rounded-xl shadow-md">
            <div className="flex flex-col items-center text-center gap-3">
              {/* <img
                src={`${BASE_URL}/${data.dailyTips?.image}`}
                alt={data.dailyTips?.title || "Daily Tip Image"}
              ...
              <div className="w-full mt-3 space-y-3">
                <InfoRow
                  IconComp={TitleIcon}
                  title="Title"
                  value={data.dailyTips?.title || "-"}
                />
                <InfoRow
                  IconComp={ImageIcon}
                  title="Daily Tip ID"
                  value={data.dailyTips?._id?.slice(-6) || "-"}
                />
                <InfoRow
                  IconComp={CalendarTodayIcon}
                  title="Show Date"
                  value={
                    data.dailyTips?.showDate
                      ? new Date(data.dailyTips.showDate).toLocaleDateString()
                      : "-"
                  }
                />
                <InfoRow
                  IconComp={VideoCameraBackIcon}
                  title="Video File"
                  value={
                    data.dailyTips?.videoFile ? (
                      <a
