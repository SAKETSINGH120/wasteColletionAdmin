...
const UpdateDailyTips = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchDailyTip = async () => {
        setLoading(true);
        try {
          const res = await getDailyTipsByIdApi(id);
          if (res.status) {
            const { title, image, videoFile, url, status, showDate } = res.data;
            setFormData({
              title: title || "",
              image: null,
              videoFile: null,
            ...
            setPreviews({
              ...
              videoFile: videoFile ? `${BASE_URL}/${videoFile}` : null,
            });
            toast.success("Daily Tip fetched successfully!");
          } else {
            toast.error(res.message || "Failed to fetch daily tip data");
          }
        } catch (err) {
          console.error("Failed to fetch daily tip data:", err);
          toast.error(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateDailyTipsApi(id, dataToSend);
      if (res.status) {
        toast.success("Daily Tip updated successfully!");
        navigate(-1);
      } else {
        toast.error(res.message || "Failed to update daily tip");
      }
    } catch (err) {
      toast.error(
        err?.response?.data?.message || err.message || "Server error"
    ...
    <div className="p-8 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
      <div className="bg-white p-6 max-w-9xl rounded-xl shadow-xl">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6">
            ...
            {previews.videoFile && (
              <p className="ml-2 mt-2 text-sm text-gray-600">
                Selected: {previews.videoFile.split("/").pop()}
              </p>
            )}
            <label
          ...
          >
            {loading ? "Updating..." : "Update Daily Tip"}
          </button>
        </form>
      </div>
    </div>
