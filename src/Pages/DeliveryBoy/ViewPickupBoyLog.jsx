...
import { getPickupBoyLogByIdApi } from "../../Services/DeliveryBoyApi";
...
export default function ViewPickupBoyLog() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const result = await getPickupBoyLogByIdApi(id);
      if (result?.status) {
        toast.success(result.message || "Pickup boy log fetched successfully!");
        setData(result.data);
      } else {
        toast.error(result?.message || "Failed to fetch pickup boy log.");
      }
    } catch (error) {
      console.error("Error fetching pickup boy log:", error);
      toast.error("Error fetching pickup boy log.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getImageUrl = (path) => {
    if (!path) return "/defaultProfile.jpg"; // fallback in public
    ...
    <div className="p-8 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
      ...
      >
        <div className="flex justify-between items-center mb-8 pb-6 border-b-2 border-gray-200 px-6 pt-6">
          <div>
            <h1 className="text-3xl font-bold text-[#1F4926] tracking-wide">
              Pickup Boy Log #{String(data._id).slice(-6)}
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 pb-6">
          {/* Left: Pickup Boy Details */}
          <div className="col-span-1 bg-white p-4 rounded-xl shadow-md">
            <div className="flex flex-col items-center text-center gap-3">
              <img
                src={`${BASE_URL}/${data.pickupBoy?.profileImage}`}
                alt={data.pickupBoy?.name || "Pickup Boy"}
                // onError={(e) => (e.target.src = "/defaultProfile.jpg")}
              ...
              <div className="w-full mt-3 space-y-3">
                <InfoRow
                  IconComp={PersonIcon}
                  title="Name"
                  value={data.pickupBoy?.name || "-"}
                />
                <InfoRow
                  IconComp={EmailIcon}
                  title="Email"
                  value={data.pickupBoy?.email || "-"}
                />
                <InfoRow
                  IconComp={PhoneIcon}
                  title="Mobile"
                  value={data.pickupBoy?.mobile || "-"}
                />
                <InfoRow
                  IconComp={ImageIcon}
                  title="Pickup Boy ID"
                  value={data.pickupBoy?._id?.slice(-6) || "-"}
                />
              </div>
            </div>
          </div>
