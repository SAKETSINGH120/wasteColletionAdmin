...
import { getPickupBoyLogsApi } from "../../Services/DeliveryBoyApi";
import { EyeIcon } from "@heroicons/react/24/outline";
import MoreVertIcon from "@mui/icons-material/MoreVert";
import Chip from "@mui/material/Chip";
import xlsx from "json-as-xlsx";

export default function PickupBoyLog() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getPickupBoyLogsApi({
        page,
        rowsPerPage,
        searchQuery,
      });
      if (result?.status) {
        toast.success("Pickup boy logs fetched successfully!");
        const transformedData = (result.data || []).map((item) => ({
          ...item,
          id: item._id,
          pickupBoyId: item.pickupBoy?._id,
          pickupBoyName: item.pickupBoy?.name,
          pickupBoyEmail: item.pickupBoy?.email,
          pickupBoyMobile: item.pickupBoy?.mobile,
        ...
        toast.error(result?.message || "Failed to fetch pickup boy logs.");
      }
    } catch (error) {
      console.error("Error fetching pickup boy logs:", error);
      toast.error("Error fetching pickup boy logs.");
    } finally {
      setLoading(false);
    }
  ...
  const exportFunc = async (allLogsData) => {
    if (allLogsData.length < 1) {
      return toast.error("Pickup boy logs list is empty!");
    }
    setIsExporting(true);
    const settings = {
      fileName: "PickupBoy_Logs",
      extraLength: 3,
      writeMode: "writeFile",
      writeOptions: {},
      RTL: false,
    ...
    const data = [
      {
        sheet: "Pickup Boy Logs",
        columns: [
          { label: "Log ID", value: (row) => row?._id || "" },
          { label: "Pickup Boy ID", value: (row) => row?.pickupBoyId || "N/A" },
          { label: "Name", value: (row) => row?.pickupBoyName || "N/A" },
          { label: "Email", value: (row) => row?.pickupBoyEmail || "N/A" },
          { label: "Mobile", value: (row) => row?.pickupBoyMobile || "N/A" },
          {
            label: "Updated By",
    ...
    try {
      xlsx(data, settings);
      toast.success("Pickup boy logs exported to Excel successfully!");
    } catch (error) {
      console.error("Error exporting pickup boy logs to Excel:", error);
      toast.error("Failed to export pickup boy logs to Excel.");
    } finally {
    ...
    <div className="p-6 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-4">
          <input
            type="text"
            placeholder="Search pickup boy logs..."
            value={search}
            ...
            aria-label="Search pickup boy logs"
          />
          <button
            onClick={() => {
        ...
        <motion.button
          ...
          aria-label="Export pickup boy logs to Excel"
        >
          {isExporting ? (
            ...
            {data.length === 0 ? (
              ...
              data.map((row, index) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-gray-600">
                    {row.pickupBoyName || "N/A"}
                  </td>
                  <td className="py-3 px-4 text-gray-600">
                    {row.pickupBoyEmail || "N/A"}
                  </td>
                  <td className="py-3 px-4 text-gray-600">
                    <Chip
                      label={row.pickupBoyMobile || "N/A"}
                      color="primary"
                      size="small"
                      variant="outlined"
                    />
                  ...
                  <td className="py-3 px-4 text-center">
                    ...
                    >
                      <button
                        onClick={() => {
                          navigate(`PickupBoyLogView/${row.id}`); // Adjust route as needed for detailed view
                          handleMenuClose();
                        }}
      ...
      {totalRecord > rowsPerPage && (
        <div className="mt-6 flex justify-center">
          <nav aria-label="Pagination for pickup boy logs">
