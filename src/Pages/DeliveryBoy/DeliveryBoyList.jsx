...
export default function DeliveryBoyList() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getAllDeliveryBoys({
        page,
        rowsPerPage,
        searchQuery,
      });
      if (result?.status) {
        toast.success("Delivery boys fetched successfully!");
  ...
  const deleteHandler = (id) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          setLoading(true);
          const result = await DeliveryBoyDelete(id);
          if (result?.status) {
            toast.success("Delivery boy deleted successfully!");
            fetchData();
          } else {
            toast.error(result?.message || "Failed to delete delivery boy.");
          }
        } catch (error) {
  ...
  const exportFunc = async (allLeadsData) => {
    if (allLeadsData.length < 1) {
      return toast.error("Delivery boy list is empty!");
    }
    setIsExporting(true);
    const settings = {
      fileName: "WasteCollection_DeliveryBoys",
      extraLength: 3,
      writeMode: "writeFile",
      writeOptions: {},
      RTL: false,
    ...
    const data = [
      {
        ...
        columns: [
          ...
          { label: "Mobile", value: (row) => row?.mobile || "" },
          {
            ...
            value: (row) => row?.profileImage || "",
          },
          { label: "Country Code", value: (row) => row?.countryCode || "" },
          ...
          },
          {
            ...
            value: (row) => row?.wallet?.lockedBalance ?? 0,
          },
          {
            label: "Verified",
            value: (row) => (row?.isVerifYed ? "Yes" : "No"),
          },
          {
            label: "Status",
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        >
          <div className="flex items-center gap-4">
            <input
              ...
              aria-label="Search delivery boys"
            />
        ...
        <div className="flex gap-4">
          <motion.button
            ...
            aria-label="Export delivery boys to Excel"
          >
            {isExporting ? (
          ...
          <motion.button
            ...
            data-aos="fade-left"
            className="bg-gradient-to-t from-[#45d85e] to-[#1F4926] text-white px-5 py-2.5 rounded-lg font-medium shadow hover:shadow-lg transition-shadow"
            aria-label="Add new delivery boy"
          >
            {isLoading ? (
              <span className="flex items-center gap-2">
                <LoderBtn />
      ...
      >
        <Table sx={{ minWidth: 700 }} aria-label="delivery boy table">
          <TableHead>
            <TableRow>
            ...
            {data.length === 0 ? (
              ...
              data.map((row, index) => (
                <StyledTableRow key={row.id}>
                  ...
                  <StyledTableCell>
                    {row.profileImage ? (
                      <img
                        ...
                        alt={`Profile of ${row.name || row.id}`}
                        onClick={() =>
                  ...
                  <StyledTableCell className="text-gray-600 capitalize">
                    {row.gender || "N/A"}
                  </StyledTableCell>
                  <StyledTableCell>
                    <span
                  ...
                  <StyledTableCell align="center">
                    <IconButton
                      ...
                      aria-label={`Actions for delivery boy ${
                        row.name || row.id
                      }`}
                    >
                      <MoreVertIcon />
                    ...
                    >
                      <MenuItem
                        onClick={() => {
                          navigate(`deliveryboyView/${row.id}`);
                          handleMenuClose();
                        }}
                      ...
                      <MenuItem
                        onClick={() => {
                          navigate(`updateDeliveryBoy/${row.id}`);
                          handleMenuClose();
                        }}
                        className="flex items-center gap-2 text-gray-700 hover:bg-gray-100"
                      >
      ...
      {totalRecord > rowsPerPage && (
        <Stack spacing={2} alignItems="center" marginTop={6}>
          <Pagination
            ...
            boundaryCount={1}
            siblingCount={1}
            aria-label="Pagination for delivery boy list"
          />
        </Stack>
      )}
    </div>
