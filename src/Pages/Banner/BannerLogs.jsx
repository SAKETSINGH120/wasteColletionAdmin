...
import { getBannerLogsApi } from "../../Services/BannerApi"; // You'll need to create this API service
...
export default function BannerLog() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getBannerLogsApi({
        page,
        rowsPerPage,
        searchQuery,
      });
      if (result?.status) {
        toast.success("Banner logs fetched successfully!");
        const transformedData = (result.data || []).map((item) => ({
          ...item,
          id: item._id,
          bannerId: item.banner?._id,
          imagePath: item.banner?.image,
          updatedByEmail: item.updatedBy?.email,
          updatedById: item.updatedBy?._id,
        ...
        toast.error(result?.message || "Failed to fetch banner logs.");
      }
    } catch (error) {
      console.error("Error fetching banner logs:", error);
      toast.error("Error fetching banner logs.");
    } finally {
      setLoading(false);
    }
  ...
  const exportFunc = async (allLogsData) => {
    if (allLogsData.length < 1) {
      return toast.error("Banner logs list is empty!");
    }
    setIsExporting(true);
    const settings = {
      fileName: "Banner_Logs",
      extraLength: 3,
      writeMode: "writeFile",
      writeOptions: {},
      RTL: false,
    ...
    const data = [
      {
        ...
        columns: [
          ...
          { label: "Banner ID", value: (row) => row?.bannerId || "N/A" },
          { label: "Image Path", value: (row) => row?.imagePath || "N/A" },
          { label: "Priority", value: (row) => row?.banner?.priority || "N/A" },
          { label: "Updated By", value: (row) => row?.updatedByEmail || "N/A" },
          {
            label: "Updated At",
            value: (row) => row?.updatedAt || "N/A",
          },
          {
    ...
    try {
      xlsx(data, settings);
      toast.success("Banner logs exported to Excel successfully!");
    } catch (error) {
      console.error("Error exporting banner logs to Excel:", error);
      toast.error("Failed to export banner logs to Excel.");
    } finally {
    ...
    <div className="p-6 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-4">
          <input
            type="text"
            placeholder="Search banner logs..."
            value={search}
        ...
        <motion.button
          ...
          aria-label="Export banner logs to Excel"
        >
          {isExporting ? (
      ...
      <div
        ...
        data-aos="fade-up"
      >
        <table className="w-full min-w-[800px]" aria-label="banner logs table">
          <thead>
                ...
                >
                  ...
                  <td className="py-3 px-4 text-center">
                    ...
                    >
                      <button
                        onClick={() => {
                          navigate(`bannerLogView/${row.id}`); // Adjust route as needed for detailed view
                          handleMenuClose();
                        }}
      ...
      {totalRecord > rowsPerPage && (
        <div className="mt-6 flex justify-center">
          <nav aria-label="Pagination for banner logs">
